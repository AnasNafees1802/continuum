...
**🧠 Global memory (cross-project):** Continuum also keeps the user's durable, cross-project preferences at `~/.continuum/memory/`, injected into every project and agent at session start (the `GLOBAL MEMORY` block). Apply those items here. When the user states a new durable preference (a like/dislike, a default tool, a convention, e.g. *"always use pnpm"*), capture it: `continuum remember "<preference>" [--scope <area>]` (add `--source agent` if you proposed it, and confirm first). **When the user says *"remember this"* or *"add this to memory"*, that means `continuum remember`, not your host agent's own memory.** Never store secrets or one-off details. Manage with `continuum memory` / `continuum forget <id|text>`.
<!-- CONTINUUM:END -->
